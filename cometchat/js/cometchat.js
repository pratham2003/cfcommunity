...
<?php $callbackfn = ''; if (!empty($_GET['callbackfn'])) { $callbackfn = $_GET['callbackfn']; }?>
        ...
        $.cometchat = $.cometchat || function(){
            ...
            ccvariable.todaysDay = ccvariable.todaysDate.getDate();
            ccvariable.currentTime = Math.floor(ccvariable.todaysDate.getTime() / 1000);
            ...
            } else if (settings.mobiletab) {
                if (ccvariable.mobileDevice || location.href.match(baseUrl + 'extensions/mobiletab/')){
                    calleeAPI = 'ccmobiletab';
                    mobile = 1;
			ccvariable.callbackfn = 'ccmobiletab';
                }
            }
            ...
            $(window).focus(function() {
                ccvariable.isMini = 0;
                if (settings.googleDesktopNotifications == 1) {
            ...
            arguments.callee.getFlashMovie = function(movieName) {
            ...
            arguments.callee.chatHeartbeat = function(force){
                ...
                if (force == 1) {
                    if (typeof window.cometcall_function == 'function') {
                        cometcall_function(cometid, 0, ccvariable.callbackfn);
                    }
                }
      
                ccvariable.externalVars["timestamp"] = ccvariable.timestamp;
                ...
                if ((ccvariable.callbackfn != '' && ccvariable.callbackfn != 'desktop') || mobile == 1) {
                ...
                $.ajax({
                    ...
                    error: function() {
                        ...
                        ccvariable.heartbeatTime = settings.minHeartbeat;
                        ccvariable.heartbeatTimer = setTimeout(function() { jqcc.cometchat.chatHeartbeat(); }, ccvariable.heartbeatTime);
                    },
                    success: function(data) {
                        if (data) {
                            $.each(data, function(type, item){
                                ...
                                if (type == 'cometid') {
                                    cometcall_function(item.id, 0, ccvariable.callbackfn);
                                }
                                if (type == 'init') {
                                ...
                                if (type == 'messages') {
                                    if (typeof(jqcc[calleeAPI].newMessages) == 'function') {
                                        newMessage = jqcc[calleeAPI].newMessages(item);
                                    }
                                }
                            });
                    ...
                    success: function(data) {
                        if (data) {
                            if(typeof(jqcc[settings.theme].createChatboxData) !=='undefined'){
                                jqcc[settings.theme].addMessage(id, message, '1', '1', data, 1, Math.floor(new Date().getTime() / 1000));
                            }
                        ...
                        ccvariable.heartbeatCount = 1;
                        if (ccvariable.heartbeatCount > settings.minHeartbeat) {
                            ccvariable.heartbeatCount = settings.minHeartbeat;
                            clearTimeout(ccvariable.heartbeatTimer);
                            ccvariable.heartbeatTimer = setTimeout(function() { jqcc.cometchat.chatHeartbeat(); }, settings.minHeartbeat);
                        }
						if (messagenext.length > 0) {
            ...
            arguments.callee.sendMessage = function (id, message) {
                if (message != '') {
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data) { 
                                if(typeof(jqcc[calleeAPI].addMessage) !=='undefined') { 
                                    jqcc[calleeAPI].addMessage(id, message, 1, 1, data, 1, 1);
                                }
            ...
            arguments.callee.tryClickSync = function (id) {
                if (ccvariable.buddylistName[id] == null || ccvariable.buddylistName[id] == '') {
                    if (ccvariable.trying[id] < 5) {
                        setTimeout(function() { jqcc.cometchat.tryClickSync(id); }, 500);
                    }
                } else {
           ...
           arguments.callee.userDoubleClick = function(listing) {
            ...
            arguments.callee.tryClick = function(id) {
                if (ccvariable.buddylistName[id] == null || ccvariable.buddylistName[id] == '') {
                    if (ccvariable.trying[id] < 5) {
                        setTimeout(function() { jqcc.cometchat.tryClick(id); }, 500);
                    }
                } else {
            ...
            arguments.callee.notify = function(title, image, message, clickEvent, id, msgid){	
				if(navigator.userAgent.match(/chrome/i)&&settings.googleDesktopNotifications==1){	             
					...
					if(Notification.permission === "granted"&&typeof title!='undefined'&&typeof image!='undefined'&&typeof message!='undefined'){
						...
						if(tempMsg.find('img.cometchat_smiley').length){
							message = language[59];
						} else {
            ...
            /*
            arguments.callee.chatWithFriend = function (id) {
                $.ajax({
                    url: baseUrl+"cometchat_isfriend.php",
                    data: {userid: id, basedata: ccvariable.baseData},
                    type: ccvariable.dataMethod,
                    timeout: ccvariable.dataTimeout,
                    cache: false,
            ...
            arguments.callee.addMessage = function (id,incomingmessage,self,old,incomingid,selfadded,sent,prepend) {
                var shouldPop = 0;
                if ($('#cometchat_user_'+id).length >0) { } else {
                    shouldPop = 1;
                }
                addMessage(id,incomingmessage,self,old,incomingid,selfadded,sent,prepend);
                if (ccvariable.openChatboxId == '' && settings.autoPopupChatbox == 1 && shouldPop == 1) {
                    tryClick(id);
                }
            };*/
        };
