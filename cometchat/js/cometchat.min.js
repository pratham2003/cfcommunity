<?php  $callbackfn='';if(!empty($_GET['callbackfn'])){$callbackfn=$_GET['callbackfn'];} ?>(function($){$.cometchat=$.cometchat||function(){var baseUrl='<?php  echo BASE_URL; ?>';var ccvariable=[];var mobile=0;<?php  echo $settings; ?>ccvariable.documentTitle=document.title;ccvariable.isJabber={};ccvariable.externalVars={};ccvariable.sessionVars={};ccvariable.internalVars={};ccvariable.openChatboxId='';ccvariable.loggedout=0;ccvariable.offline=0;ccvariable.windowFocus=true;ccvariable.resynchronize=0;ccvariable.heartbeatTimer;ccvariable.heartbeatTime=settings.minHeartbeat;ccvariable.heartbeatCount=1;ccvariable.updateSessionVars=0;ccvariable.timestamp=0;ccvariable.lastOnlineNumber=0;ccvariable.trayOpen='';ccvariable.newMessages=0;ccvariable.buddylistName={};ccvariable.buddylistMessage={};ccvariable.buddylistStatus={};ccvariable.buddylistAvatar={};ccvariable.buddylistLink={};ccvariable.buddylistIsDevice={};ccvariable.jabberOnlineNumber=0;ccvariable.chatBoxesOrder={};ccvariable.trying={};ccvariable.todaysDate=new Date();ccvariable.todaysDay=ccvariable.todaysDate.getDate();ccvariable.currentTime=Math.floor(ccvariable.todaysDate.getTime()/1000);ccvariable.specialChars=/([^\x00-\x80]+)|([&][#])+/;ccvariable.typingTo=0;ccvariable.idleTime=ccvariable.currentTime;ccvariable.idleFlag=0;ccvariable.currentStatus;ccvariable.buddyListHash;ccvariable.callbackfn='<?php  echo $callbackfn; ?>';ccvariable.baseData=$.cookie(settings.cookiePrefix+'data');ccvariable.mobileDevice=navigator.userAgent.match(/ipad|ipod|iphone|android|windows ce|blackberry|palm|symbian/i);ccvariable.initialized=0;ccvariable.crossDomain='<?php  echo CROSS_DOMAIN; ?>';ccvariable.dataMethod='get';ccvariable.dataTimeout='10000';ccvariable.isMini=0;ccvariable.desktopNotification={};ccvariable.runHeartbeat=1;ccvariable.userid=0;var calleeAPI=settings.theme;if(typeof(ccvariable.callbackfn)!='undefined'&&ccvariable.callbackfn!=''){calleeAPI=ccvariable.callbackfn;}else if(settings.mobiletab){if(ccvariable.mobileDevice||location.href.match(baseUrl+'extensions/mobiletab/')){calleeAPI='ccmobiletab';mobile=1;ccvariable.callbackfn='ccmobiletab';}}
...
$(window).focus(function(){ccvariable.isMini=0;if(settings.googleDesktopNotifications==1){for(x in ccvariable.desktopNotification){for(y in ccvariable.desktopNotification[x]){ccvariable.desktopNotification[x][y].close();}}}
...
arguments.callee.getFlashMovie=function(movieName){var isIE=navigator.appName.indexOf("Microsoft")!=-1;return(isIE)?window[movieName]:document[movieName];};arguments.callee.chatHeartbeat=function(force){var newMessage=0;if(force==1){if(typeof window.cometcall_function=='function'){cometcall_function(cometid,0,ccvariable.callbackfn);}}
ccvariable.externalVars["timestamp"]=ccvariable.timestamp;ccvariable.externalVars["typingto"]=ccvariable.typingTo;ccvariable.externalVars["blh"]=ccvariable.buddyListHash;ccvariable.externalVars["status"]="";if((ccvariable.callbackfn!=''&&ccvariable.callbackfn!='desktop')||mobile==1){ccvariable.externalVars["status"]='available';}
...
ccvariable.externalVars["basedata"]=ccvariable.baseData;$.ajax({url:baseUrl+"cometchat_receive.php",data:ccvariable.externalVars,dataType:'jsonp',type:ccvariable.dataMethod,timeout:ccvariable.dataTimeout,error:function(){clearTimeout(ccvariable.heartbeatTimer);ccvariable.heartbeatTime=settings.minHeartbeat;ccvariable.heartbeatTimer=setTimeout(function(){jqcc.cometchat.chatHeartbeat();},ccvariable.heartbeatTime);},success:function(data){if(data){$.each(data,function(type,item){if(type=='blh'){ccvariable.buddyListHash=item;}
...
if(type=='cometid'){cometcall_function(item.id,0,ccvariable.callbackfn);}
...
if(type=='messages'){if(typeof(jqcc[calleeAPI].newMessages)=='function'){newMessage=jqcc[calleeAPI].newMessages(item);}}});if(ccvariable.openChatboxId!=''&&newMessage>0){if(typeof(jqcc[calleeAPI].tryAddMessages)!=='undefined'){jqcc[calleeAPI].tryAddMessages(ccvariable.openChatboxId,atleastOneNewMessage);}}
