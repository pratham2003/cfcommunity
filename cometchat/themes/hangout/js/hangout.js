        ...
        return {
            ...
            initialize: function() {
                    ...
                    for (x in trayicon) {
                         if(x != 'home' && x != 'scrolltotop') {
                             if (trayicon.hasOwnProperty(x)) {
                                 var icon = trayicon[x];
                                 var onclick = 'onclick="jqcc.cometchat.lightbox(\''+x+'\')"'; 
                ...
                if(settings.showSettingsTab == 0){
                    ...
                    $('#cometchat_userstab_popup .cometchat_tabstyle').addClass('cometchat_border_bottom');
                }
                if(settings.showSettingsTab == 1 && settings.showOnlineTab == 0){
                    $('#cometchat_optionsbutton').click(function(){
                       jqcc( "#cometchat_chatboxes_wide span" ).each(function( index ) {
            ...
            newAnnouncement: function(item) {
                if ($.cookie(settings.cookiePrefix + "popup") && $.cookie(settings.cookiePrefix + "popup") == 'true') { } else {
                    ...
                    jqcc[settings.theme].tooltip("cometchat_optionsbutton", message, 0);
            ...
            buddyList: function(item) { 
                ...
                $.each(item, function(i, buddy) {
                    ...
                    if (chatboxOpened[buddy.id] != null) {
                        $("#cometchat_user_"+buddy.id+"_popup .cometchat_tabtitle")
                ...
                if (tooltipMessage != '' && !$('#cometchat_userstab_popup').hasClass('cometchat_tabopen') && !$('#cometchat_optionsbutton_popup').hasClass('cometchat_tabopen')) {
                    if (tooltipPriority < 10) {
                        if ($.cookie(settings.cookiePrefix + "popup") && $.cookie(settings.cookiePrefix + "popup") == 'true') {
                            ...
                            tooltipPriority = 108;
                            jqcc[settings.theme].tooltip("cometchat_optionsbutton", tooltipMessage, 0);
                            clearTimeout(notificationTimer);
                ...
                $.each(item, function(i, incoming) {                                 
                    ...
                    if ((jqcc.cometchat.getThemeVariable('openChatboxId')) == (incoming.from) && jqcc.cometchat.getThemeArray('buddylistName',incoming.from) != '' && jqcc.cometchat.getThemeArray('buddylistName',incoming.from) != null) {
                       ...
                       if (incoming.self == 1) {
                        ...
                        if ($("#cometchat_message_" + incoming.id).length > 0) {
                            $("#cometchat_message_" + incoming.id).html(incoming.message);
                        } else {
                            ...
                            if (incoming.self == 1) {
                                if(jqcc.cometchat.getThemeArray('buddylistAvatar',jqcc.cometchat.getThemeVariable('userid')) != "") {
                                    var avatar = jqcc.cometchat.getThemeArray('buddylistAvatar',jqcc.cometchat.getThemeVariable('userid'));
                                } else {
                                    var avatar = baseUrl+"/themes/hangout/images/noavatar.png";
                                }
                                if($('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last').hasClass('self')){
                                    if($('#cometchat_message_' + incoming.id).length > 0) {
                                        $('#cometchat_message_' + incoming.id).html(incoming.message);
                                    } else {
                                         if(!$("#cometchat_message_"+incoming.id).length) {
                                             $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_ts').before('<div id="cometchat_message_' + incoming.id + '" class="'+selfstyle+' cometchat_other_noarrow">'+incoming.message+'</div>');
                                         }
                                    }
                                    $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_ts').html(jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self));
                                } else {
                                    $('#cometchat_tabcontenttext_'+incoming.from).append('<div class="cometchat_chatboxmessage self"><div class="cometchat_chatboxmessagecontent '+selfstyleCont+'"><div id="cometchat_message_' + incoming.id + '" class="'+selfstyle+'">'+incoming.message+'</div><span class="cometchat_ts">'+jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self)+'</span></div><div class="cometchat_chatboxmessagefrom '+selfstyleAvatar+'"><a href="'+ jqcc.cometchat.getThemeArray("buddylistLink",jqcc.cometchat.getThemeVariable("userid"))+'"><img src="'+avatar+'" title="'+fromname+'"/></a></div></div>');
                                }
                            } else {
                                if(jqcc.cometchat.getThemeArray('buddylistAvatar',incoming.from) != "") {
                                    var avatar = jqcc.cometchat.getThemeArray('buddylistAvatar',incoming.from);
                                } else {
                                    var avatar =  baseUrl+"themes/hangout/images/noavatar.png";
                                }
                                if($('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last').hasClass('other')){
                                     if(!$("#cometchat_message_"+incoming.id).length) {
                                        $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_message_name').before('<div id="cometchat_message_' + incoming.id + '" class="'+selfstyle+' cometchat_other_noarrow">'+incoming.message+'</div>');
                                        $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_ts').html(jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self));
                                     }
                                } else {
                                    $('#cometchat_tabcontenttext_'+incoming.from).append('<div class="cometchat_chatboxmessage other" ><div class="cometchat_chatboxmessagefrom '+selfstyleAvatar+'"><a href="'+ jqcc.cometchat.getThemeArray('buddylistLink',incoming.from)+'"><img src="'+avatar+'" title="'+fromname+'"/></a></div><div class="cometchat_chatboxmessagecontent '+selfstyleCont+' <strong>.</strong>"><div id="cometchat_message_' + incoming.id + '" class="'+selfstyle+'">'+incoming.message+'</div><span class="cometchat_message_name">'+fromname+' <strong>.</strong></span><span class="cometchat_ts">'+jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self)+'</span></div></div>');
                                }
                            }
                    ...
                    if(newMessage > 0){
                        if($('#cometchat_tabcontenttext_' + incoming.from).outerHeight()< totalHeight){
                            $('#cometchat_tabcontenttext_' + incoming.from).append('<div class="cometchat_new_message_unread"><a herf="javascript:void(0)" onClick="javascript:jqcc.hangout.scrollDown('+incoming.from+');jqcc(\'#cometchat_tabcontenttext_' + incoming.from +' .cometchat_new_message_unread\').remove();">&#9660 '+language[54]+'</a></div>');
                        }
                    }
                });
            ...
            createChatboxData: function(id, name, status, message, avatar, link, silent, tryOldMessages) { 
                ...
                $("<span/>").attr("id", "cometchat_user_" + id).attr("amount", 0).addClass("cometchat_tab").html('<div class="cometchat_icon"></div><div class="cometchat_user_shortname">' + shortname + '</div>').appendTo($("#cometchat_chatboxes_wide"));
                ...
                $("<div/>").attr("id", "cometchat_user_" + id + "_popup").addClass("cometchat_tabpopup").css('display', 'none').html('<div class="cometchat_tabtitle"><div class="cometchat_icon"></div><div class="cometchat_name">' + startlink + longname + endlink + '<span id="cometchat_typing_' + id + '" class="cometchat_typing"></span></div></div><div class="cometchat_tabsubtitle">' + pluginshtml + '<div style="clear:both"></div>' + '</div><div class="cometchat_tabcontent"><div class="cometchat_tabcontenttext" id="cometchat_tabcontenttext_' + id + '"></div><div class="cometchat_tabcontentinput"><div class="cometchat_pluginsicon cometchat_smilies" onclick="javascript:jqcc.ccsmilies.init(' + id + ');"></div><textarea class="cometchat_textarea" placeholder="'+language[55]+'"></textarea><div class="cometchat_tabcontentsubmit"></div></div><div style="clear:both"></div></div>').appendTo($("#cometchat"));
                
                if (jqcc().slimScroll) {
                    $("#cometchat_user_" + id + "_popup .cometchat_tabcontenttext").slimScroll({height: '211px'});
                }
                chatbottom[id] = 1;
                $("#cometchat_user_" + id + "_popup .cometchat_tabcontenttext").scroll(function() {
                    if($("#cometchat_user_" + id + "_popup .cometchat_tabcontenttext").height() < ($("#cometchat_user_" + id + "_popup .slimScrollBar").height() + $("#cometchat_user_" + id + "_popup .slimScrollBar").position().top)){
                        chatbottom[id] = 1;
                        $('#cometchat_tabcontenttext_' + id+' .cometchat_new_message_unread').remove();
                    } else {
                        chatbottom[id] = 0;
                    }
                });
                if(status == 'available') {
                    $("#cometchat_user_"+id+"_popup .cometchat_tabtitle").addClass('cometchat_tabtitle_available');
                } else if(status == 'away') {
                    $("#cometchat_user_"+id+"_popup .cometchat_tabtitle").addClass('cometchat_tabtitle_away');
                } else if(status == 'busy') {
                    $("#cometchat_user_"+id+"_popup .cometchat_tabtitle").addClass('cometchat_tabtitle_busy');
                } else {
                    $("#cometchat_user_"+id+"_popup .cometchat_tabtitle").addClass('cometchat_tabtitle_offline');
                }
                ...
                $("#cometchat_user_" + id).click(function() {
                    ...
                    if ($(this).hasClass('cometchat_tabclick')) {
                        ...
                        if (($("#cometchat_user_" + id).offset().left < ($("#cometchat_chatboxes").offset().left + $("#cometchat_chatboxes").width())) && ($("#cometchat_user_" + id).offset().left - $("#cometchat_chatboxes").offset().left) >= 0) {
                            ...
                            $("#cometchat_user_" + id + '_popup').css('left', baseLeft + $("#cometchat_user_" + id).position().left + 1 );
                            ...
                            var newPosition = (($("#cometchat_user_" + id).offset().left - $("#cometchat_chatboxes_wide").offset().left)) - ((Math.floor(($('#cometchat_chatboxes').width() / 230)) - 6) * 230 +5);
            ...
            addMessage: function(id, incomingmessage, self, old, incomingid, selfadded, sent) {
				...
				if (jqcc.cometchat.getThemeArray('buddylistName',id) == null || jqcc.cometchat.getThemeArray('buddylistName',id) == ''){
					...
					if ($("#cometchat_message_" + incomingid).length > 0) {
						$("#cometchat_message_" + incomingid).html(incomingmessage);
					} else {
						sentdata = '';

						if (sent != null) {
							var ts = new Date(sent * 1000);
							sentdata = jqcc[settings.theme].getTimeDisplay(ts, id,1);
						}
						...
						if (parseInt(self) == 1) {
							...
							if($('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last').hasClass('self')){
								if($("#cometchat_message_"+incomingid).length > 0) {
									$("#cometchat_message_"+incomingid).html(incomingmessage);
								} else {
									if($('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last.self .cometchat_chatboxmessagecontent  ').hasClass('cometchat_self_content')){ 
										$('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last.self .cometchat_self_content .cometchat_ts').before('<div id="cometchat_message_'+incomingid+'" class="'+selfstyle+'">'+incomingmessage+'</div>');
									}
								}
								$('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last .cometchat_self_content .cometchat_ts').html(sentdata);
							} else {
								msg = jqcc[settings.theme].processMessage('<div class="cometchat_chatboxmessage self" ><div class="cometchat_chatboxmessagecontent '+selfstyleCont+'"><div id="cometchat_message_'+incomingid+'" class="'+selfstyle+'">'+incomingmessage+'</div><span class="cometchat_ts">'+sentdata+'</span></div><div class="cometchat_chatboxmessagefrom '+selfstyleAvatar+'"><a href="'+jqcc.cometchat.getThemeArray('buddylistLink',jqcc.cometchat.getThemeVariable('userid'))+'"><img src="'+avatar+'" title="'+fromname+'"/></a></div></div>',selfstyle);
								addMessage = 1;
							}
						} else {
							if(jqcc.cometchat.getThemeArray('buddylistAvatar',id) != "") {
							 ...
							 if($('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last').hasClass('other')){
								 if($("#cometchat_message_"+incomingid).length > 0) {
									$("#cometchat_message_"+incomingid).html(incomingmessage);
								} else {
									if($('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last.other .cometchat_chatboxmessagecontent  ').hasClass('cometchat_other_content')) {                                         
										$('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last .cometchat_other_content .cometchat_message_name').before('<div id="cometchat_message_'+incomingid+'" class="'+selfstyle+' cometchat_other_noarrow">'+incomingmessage+'</div>');
									}
								}
								 $('#cometchat_tabcontenttext_'+id+' .cometchat_chatboxmessage:last .cometchat_other_content .cometchat_ts').html(sentdata);
							 } else {
								 msg = (jqcc[settings.theme].processMessage('<div class="cometchat_chatboxmessage other"><div class="cometchat_chatboxmessagefrom '+selfstyleAvatar+'"><a href="'+jqcc.cometchat.getThemeArray('buddylistLink',id)+'"><img src="'+avatar+'" title="'+fromname+'"/></a></div><div class="cometchat_chatboxmessagecontent '+selfstyleCont+'"><div id="cometchat_message_'+incomingid+'" class="'+selfstyle+'">'+incomingmessage+'</div><span class="cometchat_message_name">'+fromname+' <strong>.</strong></span><span class="cometchat_ts">'+sentdata+'</span></div></div>',selfstyle));
								addMessage = 1;
							 }
						 }
            ...
            tryAddMessages: function(id,atleastOneNewMessage) {
                if (jqcc.cometchat.getThemeArray('buddylistName',id) == null || jqcc.cometchat.getThemeArray('buddylistName',id)  == '') {
                    ...
                    if(chatbottom[id] == 1){
                        ...
                        $('#cometchat_tabcontenttext_' + id + ' .cometchat_new_message_unread').show();
                    }
                    chatboxOpened[id] = 1;
            ...
            addPopup: function(id, amount, add) {
                if (jqcc.cometchat.getThemeArray('buddylistName',id) == null || jqcc.cometchat.getThemeArray('buddylistName',id) == '') {
                    ...
                    if (amount == 0) {
                        $("#cometchat_user_" + id ).removeClass('cometchat_new_message').attr('amount',0);
                    } else {
                        $("#cometchat_user_" + id).addClass('cometchat_new_message').attr('amount',amount);
                    }
                    $("#cometchat_user_" + id ).click(function(){
                          $("#cometchat_user_" + id ).removeClass('cometchat_new_message').attr('amount',0);
            ...
            getTimeDisplay: function(ts, id, self) {
                var dot = "<strong>.</strong>";               
                if(self) {                   
                    dot = " ";
                }
                var style = "style=\" float: left;\""; if (typeof(jqcc.ccchattime) != 'undefined' && jqcc.ccchattime.getEnabled(id)){style = "style=\"display:inline; float: left;\""};
                var ap = "";
                ...
                if (date != jqcc.cometchat.getThemeVariable('todaysDay')) {
                    return months[month] + ' '+ date+', '+ hour + ":" + minute + ' '+  ap;
                } else {
            ...
            chatboxKeyup: function(event, chatboxtextarea, id) {
                ...
                if (maxHeight > adjustedHeight) {
                    ...
                    if (adjustedHeight > chatboxtextarea.clientHeight) {
                        ...
                        $("#cometchat_user_" + id + "_popup .cometchat_tabcontenttext").css('height', 218 - (adjustedHeight + 4) + 8 + 'px');
                        $("#cometchat_user_" + id + "_popup .slimScrollDiv").css('height', 229 - (adjustedHeight + 4) + 8 + 'px');
                    }
                } else {
            ...
            chatboxKeydown: function(event, chatboxtextarea, id, force) {
                ...
                if ((event.keyCode == 13 && event.shiftKey == 0) || force == 1)  {
                    ...
                    $("#cometchat_user_" + id + "_popup .slimScrollDiv").css('height', '210px');
                    $("#cometchat_user_" + id + "_popup .cometchat_tabcontenttext").css('height', '199px');
                    ...
                    if (type == 'messages') {
                        $.each(item, function(i, incoming) {                            
                            ...
                            if (incoming.self == 1) {
                                ...
                                if($('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last').hasClass('self')){
                                    $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_self_content .cometchat_ts').before('<div id="cometchat_message_'+incoming.id+'" class="'+selfstyle+'">'+incoming.message+'</div>');
                                    $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_self_content .cometchat_ts').html(jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self));
                                    jqcc[settings.theme].scrollDown(id);
                                } else {
                                    $("#cometchat_tabcontenttext_" + id).append((jqcc[settings.theme].processMessage('<div class="cometchat_chatboxmessage self"><div class="cometchat_chatboxmessagecontent '+selfstyleCont+'"><div id="cometchat_message_'+incoming.id+'" class="'+selfstyle+'">'+incoming.message+'</div><span class="cometchat_ts">'+jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self)+'</span></div><div class="cometchat_chatboxmessagefrom '+selfstyleAvatar+'"><a href="'+jqcc.cometchat.getThemeArray('buddylistLink',jqcc.cometchat.getThemeVariable('userid'))+'"><img src="'+avatar+'" title="'+fromname+'"/></a></div></div>',selfstyle)));
                                    jqcc[settings.theme].scrollDown(id);
                                }
                            } else {
                                if(jqcc.cometchat.getThemeArray('buddylistAvatar',incoming.from) != "") {
                                ...
                                if($('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last').hasClass('other')){
                                    if(!$("#cometchat_message_"+incoming.id).length) {
                                        $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_other_content .cometchat_message_name').before('<div id="cometchat_message_'+incoming.id+'" class="'+selfstyle+' cometchat_other_noarrow">'+incoming.message+'</div>');
                                        $('#cometchat_tabcontenttext_'+incoming.from+' .cometchat_chatboxmessage:last .cometchat_other_content .cometchat_ts').html(jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self));
                                        jqcc[settings.theme].scrollDown(id);
                                    }
                                } else {
                                    $("#cometchat_tabcontenttext_" + id).append((jqcc[settings.theme].processMessage('<div class="cometchat_chatboxmessage other" ><div class="cometchat_chatboxmessagefrom '+selfstyleAvatar+'"><a href="'+jqcc.cometchat.getThemeArray('buddylistLink',incoming.from)+'"><img src="'+jqcc.cometchat.getThemeArray('buddylistAvatar',incoming.from)+'" title="'+fromname+'"/></a></div><div class="cometchat_chatboxmessagecontent '+selfstyleCont+'"><div id="cometchat_message_'+incoming.id+'" class="'+selfstyle+'">'+incoming.message+'</div><span class="cometchat_message_name">'+fromname+' <strong>.</strong></span><span class="cometchat_ts">'+jqcc[settings.theme].getTimeDisplay(ts, incoming.from,incoming.self)+'</span></div></div>',selfstyle)));
                                    jqcc[settings.theme].scrollDown(id);
                                }
                            }
                        });
            ...
            windowResize: function(silent) {
               ...
               if($('#cometchat_base').length) {
                ...
                if (jqcc.cometchat.getThemeVariable('openChatboxId') != '' && silent != 1) {
                    if (($("#cometchat_user_" + jqcc.cometchat.getThemeVariable('openChatboxId')).offset().left < ($("#cometchat_chatboxes").offset().left + $("#cometchat_chatboxes").width())) && ($("#cometchat_user_" + jqcc.cometchat.getThemeVariable('openChatboxId')).offset().left - $("#cometchat_chatboxes").offset().left) >= 0) {
                        $("#cometchat_user_" + jqcc.cometchat.getThemeVariable('openChatboxId') + '_popup').css('left', baseLeft + $("#cometchat_user_" + jqcc.cometchat.getThemeVariable('openChatboxId')).offset().left - 20);
                    } else {
                        $('#cometchat_user_' + jqcc.cometchat.getThemeVariable('openChatboxId') + '_popup').removeClass('cometchat_tabopen');
                        $('#cometchat_user_' + jqcc.cometchat.getThemeVariable('openChatboxId')).removeClass('cometchat_tabclick').removeClass("cometchat_usertabclick");
                        var newPosition = (($("#cometchat_user_" + jqcc.cometchat.getThemeVariable('openChatboxId')).offset().left - $("#cometchat_chatboxes_wide").offset().left)) - ((Math.floor(($('#cometchat_chatboxes').width() / 230)) - 1) * 230);
                        $('#cometchat_chatboxes').scrollToCC(newPosition + 'px', 0, function() {
                            $("#cometchat_user_" + jqcc.cometchat.getThemeVariable('openChatboxId')).click();
                        });
                    }
