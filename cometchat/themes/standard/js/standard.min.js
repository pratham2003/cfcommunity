...
(function($){$.standard=(function(){var settings={};var baseUrl;var language;var trayicon;var message_temp=new Object();var typingTimer;var resynchTimer;var notificationTimer;var chatboxOpened={};var trayWidth=0;var ie6=false;var siteOnlineNumber=0;var olddata={};var tooltipPriority=0;var desktopNotifications={};var webkitRequest=0;var lastmessagetime=Math.floor(new Date().getTime());var favicon;return{playSound:function(){try{jqcc.cometchat.getFlashMovie("cometchatbeep").beep();}catch(error){messageBeep=0;}},initialize:function(){settings=jqcc.cometchat.getSettings();baseUrl=jqcc.cometchat.getBaseUrl();language=jqcc.cometchat.getLanguageAll();trayicon=jqcc.cometchat.getTrayicon();var trayData='';var trayDataBody='';if(settings.windowFavicon==1){favicon=new Favico({animation:'pop'});}
...
if(jqcc.cometchat.getThemeVariable('trayOpen')==''){$('#cometchat_trayicon_'+id+'_popup').css('left',$('#cometchat_trayicon_'+id).offset().left-1).css('width',trayicon[id][4]);if(ie6){$('#cometchat_trayicon_'+id+'_popup .cometchat_traytitle').css('width',parseInt(trayicon[id][4])-5);}
if($("#cometchat_trayicon_"+id+'_iframe').attr('src')===undefined||$("#cometchat_trayicon_"+id+'_iframe').attr('src')=='blank.html'||$("#cometchat_trayicon_"+id+'_iframe').attr('src')==''){$("#cometchat_trayicon_"+id+'_iframe').attr('src',trayicon[id][2]+'?basedata='+jqcc.cometchat.getThemeVariable('baseData'));}
...
if($.cookie(settings.cookiePrefix+'hidebar')=='1'){$('#cometchat').css('display','none');$('#cometchat_hidden').css('display','block');jqcc.cometchat.setThemeVariable('runHeartbeat',0);}
...
if((jqcc.cometchat.getThemeVariable('openChatboxId'))==(incoming.from)&&jqcc.cometchat.getThemeArray('buddylistName',incoming.from)!=''&&jqcc.cometchat.getThemeArray('buddylistName',incoming.from)!=null){++newMessage;var name=jqcc.cometchat.getThemeArray('buddylistName',incoming.from);var selfstyle='';if(incoming.self==1){fromname=language[10];selfstyle=' cometchat_self';}else{fromname=name;}
if($("#cometchat_message_"+incoming.id).length>0){$("#cometchat_message_"+incoming.id+' .cometchat_chatboxmessagecontent').html(incoming.message);}else{var ts=new Date(incoming.sent*1000);if(!settings.fullName){fromname=fromname.split(" ")[0];}
$('#cometchat_tabcontenttext_'+incoming.from).append(jqcc[settings.theme].processMessage('<div class="cometchat_chatboxmessage" id="cometchat_message_'+incoming.id+'"><span class="cometchat_chatboxmessagefrom'+selfstyle+'"><strong>'+fromname+'</strong>:&nbsp;&nbsp;</span><span class="cometchat_chatboxmessagecontent'+selfstyle+'">'+incoming.message+'</span>'+jqcc[settings.theme].getTimeDisplay(ts,incoming.from)+'</div>',selfstyle));if(incoming.old==0&&incoming.self==0){atleastOneNewMessage=1;}}}else{var shouldPop=0;if($('#cometchat_user_'+incoming.from).length>0){}else{shouldPop=1;}
...
$('#cometchat_chatboxes_wide').width($('#cometchat_chatboxes_wide').width()+152);jqcc[settings.theme].windowResize(1);shortname=name;longname=name;$("<span/>").attr("id","cometchat_user_"+id).addClass("cometchat_tab").html('<div class="cometchat_user_shortname">'+shortname+'</div>').appendTo($("#cometchat_chatboxes_wide"));$("#cometchat_user_"+id).append('<div class="cometchat_closebox_bottom_status cometchat_'+status+'"></div>');$("#cometchat_user_"+id).append('<div class="cometchat_closebox_bottom"></div>');$("#cometchat_user_"+id+" .cometchat_closebox_bottom").mouseenter(function(){$(this).addClass("cometchat_closebox_bottomhover");});$("#cometchat_user_"+id+" .cometchat_closebox_bottom").mouseleave(function(){$(this).removeClass("cometchat_closebox_bottomhover");});$("#cometchat_user_"+id+" .cometchat_closebox_bottom").click(function(){$("#cometchat_user_"+id+"_popup").remove();$("#cometchat_user_"+id).remove();if(jqcc.cometchat.getThemeVariable('openChatboxId')==id){jqcc.cometchat.setThemeVariable('openChatboxId','');jqcc.cometchat.setSessionVariable('openChatboxId',jqcc.cometchat.getThemeVariable('openChatboxId'));}
...
if($("#cometchat_user_"+id+" .cometchat_tabalert").length>0){$("#cometchat_user_"+id+" .cometchat_tabalert").remove();jqcc.cometchat.setThemeArray('chatBoxesOrder',id,0);chatboxOpened[id]=0;jqcc.cometchat.orderChatboxes();}
...
$("#cometchat_user_"+id+'_popup').css('left',baseLeft+$("#cometchat_user_"+id).position().left-77);$(this).addClass("cometchat_tabclick").addClass("cometchat_usertabclick");$("#cometchat_user_"+id+'_popup').addClass("cometchat_tabopen");$("#cometchat_user_"+id+' .cometchat_closebox_bottom').addClass("cometchat_closebox_bottom_click");jqcc.cometchat.setThemeVariable('openChatboxId',id);jqcc.cometchat.setSessionVariable('openChatboxId',jqcc.cometchat.getThemeVariable('openChatboxId'));if(olddata[id]!=1&&(jqcc.cometchat.getExternalVariable('initialize')!=1||isNaN(id))){jqcc[settings.theme].updateChatbox(id);olddata[id]=1;}
if(ie6){jqcc[settings.theme].scrollFix();}}else{$('#cometchat_user_'+id+'_popup').removeClass('cometchat_tabopen');$('#cometchat_user_'+id).removeClass('cometchat_tabclick').removeClass("cometchat_usertabclick");var newPosition=(($("#cometchat_user_"+id).offset().left-$("#cometchat_chatboxes_wide").offset().left))-((Math.floor(($('#cometchat_chatboxes').width()/152))-1)*152);$('#cometchat_chatboxes').scrollToCC(newPosition+'px',0,function(){jqcc[settings.theme].checkPopups();jqcc[settings.theme].scrollBars();$("#cometchat_user_"+id).click();});}
...
jqcc.cometchat.setInternalVariable('updatingsession','0');clearTimeout(resynchTimer);resynchTimer=setTimeout(function(){jqcc[settings.theme].resynch();},5000);}},setModuleAlert:function(id,number){if(!$('#cometchat_trayicon_'+id+'_popup').hasClass('cometchat_tabopen')){$("#cometchat_trayicon_"+id+" .cometchat_tabalert").remove();if(number!=0){$("<div/>").css('top','-10px').addClass("cometchat_tabalert").html(number).appendTo($("#cometchat_trayicon_"+id));}}},addPopup:function(id,amount,add){if(jqcc.cometchat.getThemeArray('buddylistName',id)==null||jqcc.cometchat.getThemeArray('buddylistName',id)==''){if(jqcc.cometchat.getThemeArray('trying',id)===undefined){jqcc[settings.theme].createChatbox(id,null,null,null,null,null,1,null);}
if(jqcc.cometchat.getThemeArray('trying',id)<5){setTimeout(function(){jqcc[settings.theme].addPopup(id,amount,add);},5000);}}else{jqcc.cometchat.userDoubleClick(id);if(add==1){if($("#cometchat_user_"+id+" .cometchat_tabalert").length>0){amount=parseInt($("#cometchat_user_"+id+" .cometchat_tabalert").html())+parseInt(amount);}}
if(amount==0){$("#cometchat_user_"+id+" .cometchat_tabalert").remove();}else{if($("#cometchat_user_"+id+" .cometchat_tabalert").length>0){$("#cometchat_user_"+id+" .cometchat_tabalert").html(amount);}else{$("<div/>").css('top','-5px').addClass("cometchat_tabalert").html(amount).appendTo($("#cometchat_user_"+id+' .cometchat_closebox_bottom_status'));}}
jqcc.cometchat.setThemeArray('chatBoxesOrder',id,amount);jqcc.cometchat.orderChatboxes();jqcc[settings.theme].checkPopups();}},getTimeDisplay:function(ts,id){var style="style=\"display:none;\"";if(typeof(jqcc.ccchattime)!='undefined'&&jqcc.ccchattime.getEnabled(id)){style="style=\"display:inline;\""};var ap="";var hour=ts.getHours();var minute=ts.getMinutes();var date=ts.getDate();var month=ts.getMonth();if(settings.armyTime!=1){if(hour>11){ap="pm";}else{ap="am";}
...
$("#cometchat_chatboxes_wide .cometchat_tabalert").css('display','none');var baseLeft=$('#cometchat_base').position().left;var ms=settings.scrollTime;if(jqcc.cometchat.getExternalVariable('initialize')==1){ms=0;}
...
if(jqcc.cometchat.getThemeVariable('loggedout')==0){if(jqcc.cometchat.getThemeVariable('offline')==1){jqcc.cometchat.setThemeVariable('offline',0);$('#cometchat_userstab_text').html(language[9]+' ('+jqcc.cometchat.getThemeVariable('lastOnlineNumber')+')');jqcc.cometchat.chatHeartbeat(1);$("#cometchat_optionsbutton_popup .available").click();}
...
adjustedHeight=Math.min(maxHeight,adjustedHeight);if(adjustedHeight>chatboxtextarea.clientHeight){$(chatboxtextarea).css('height',adjustedHeight+4+'px');$("#cometchat_user_"+id+"_popup .cometchat_tabcontenttext").css('height',218-(adjustedHeight+4)+'px');$("#cometchat_user_"+id+"_popup .slimScrollDiv").css('height',229-(adjustedHeight+4)+'px');}}else{$(chatboxtextarea).css('overflow-y','auto');}},chatboxKeydown:function(event,chatboxtextarea,id,force){var condition=1;if((event.keyCode==13&&event.shiftKey==0)||force==1){var message=$(chatboxtextarea).val();message=message.replace(/^\s+|\s+$/g,"");$(chatboxtextarea).val('');$(chatboxtextarea).css('height','18px');$("#cometchat_user_"+id+"_popup .slimScrollDiv").css('height','211px');$("#cometchat_user_"+id+"_popup .cometchat_tabcontenttext").css('height','200px');$(chatboxtextarea).css('overflow-y','hidden');$(chatboxtextarea).focus();if(settings.floodControl){condition=((Math.floor(new Date().getTime()))-lastmessagetime>2000);}
...
if(settings.barType=='fixed'){$('#cometchat_base').css('width',settings.barWidth);if(settings.barAlign=='center'){var distance=(baseWidth-settings.barWidth)/2;$('#cometchat_base').css('left',distance);}
