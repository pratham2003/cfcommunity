...
class MGates_Plugin_Updater
{
	...
	public $api_key = ''; // Unique value, used to determine the plugin to be updated.
	...
	function admin_screen()
		...
		<div class="wrap">
			...
			<form name="<?php echo $this->plugin_token; ?>-login" id="<?php echo $this->plugin_token; ?>-login" action="<?php echo admin_url( 'index.php?page=' . $this->plugin_token ); ?>" method="post">
				<?php wp_nonce_field( $this->plugin_token . '-nonce', $this->plugin_prefix . 'nonce' ); ?>
				<fieldset>
					<table class="form-table">
						<tbody>
						<?php $i=0; foreach ( self::$plugins as $api_key => $info ) { $i++;
			$value = '';
			...
			if ( !empty( $_POST['license_keys'][$api_key] ) ) {
				$value = $_POST['license_keys'][$api_key];
							...
							<tr>
								<th scope="row"><label for="license_key-<?php echo $i;?>"><?php echo $info['title'] ?></label></th>
								<td><input type="text" class="input-text input-license regular-text" <?php echo !$valid ? 'style="border: 1px solid #B94A48;"' : 'style="border: 1px solid #468847;"'; ?> name="license_keys[<?php echo $api_key;?>]" id="license_key-<?php echo $i;?>" value="<?php echo $value; ?>" /></td>
							</tr>
						<?php } ?>
						</tbody>
					</table>
				...
				<fieldset>
					<p class="submit">
						<button type="submit" name="<?php echo $this->plugin_token; ?>-login" id="<?php echo $this->plugin_token; ?>-login" class="button-primary"><?php _e( 'Save', 'geczy' ); ?></button>
						<button type="submit" name="<?php echo $this->plugin_token; ?>-update-now" id="<?php echo $this->plugin_token; ?>-login" class="button"><?php _e( 'Check for updates <b>now</b>', 'geczy' ); ?></button>
					</p>
				</fieldset>
			</form>
...
<?php
	...
	function admin_notice()
	{
		...
		$hide_notice = get_option( $this->plugin_prefix . 'hide-admin-notice', false );

		if ( !empty( self::$keys ) ) {
			foreach ( self::$keys as $value ) if ( $value['status'] ) { $hide_notice = true; break; }
		} else {
			$hide_notice = false;
		}
		...
		if ( !$is_settings_page && !$hide_notice ) {
			...
			$notice .= '<p class="alignright submitbox"><a href="?' . $this->plugin_token . '-hide-updatenotice=true" class="submitdelete">' . __( 'Hide This Message', 'geczy' ) . '</a></p>' . "\n";
	...
	function hide_admin_notice()
	{
		if ( isset( $_GET[$this->plugin_token . '-hide-updatenotice'] ) && ( $_GET[$this->plugin_token . '-hide-updatenotice'] == 'true' ) ) {
			...
			$url = str_replace( $this->plugin_token . '-hide-updatenotice=true', '', $url );
