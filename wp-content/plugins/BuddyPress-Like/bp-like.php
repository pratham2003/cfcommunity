<?php
/*
Plugin Name: **** SMEAK Changed ****  BuddyPress Like
...
function bp_like_add_user_like( $item_id = '', $type = 'activity' ) {
	...
	if ( $type == 'activity' ) :
		...
		if ( bp_like_get_settings( 'post_to_activity_stream' ) == 1 ) {
			...
			if ( !bp_activity_get_meta( $item_id, 'bp_activity_bump_date_recorded') )
			bp_activity_update_meta( $item_id, 'bp_activity_bump_date_recorded', $activity_parent->date_recorded );

			//$activity = new BP_Activity_Activity( $item_id );
			//print_r($activity->date_recorded);
			//$activity->date_recorded = gmdate( "Y-m-d H:i:s" );
			//print_r($activity->date_recorded);
			global $wpdb;
			
			$wpdb->query($wpdb->prepare("UPDATE `wp_bp_activity` SET date_recorded = %s WHERE id = %d", gmdate( "Y-m-d H:i:s" ), $item_id));
...
function bp_like_get_likes( $item_id = '', $type = '', $user_id = '' ) {
	...
	if ( $type == 'activity' ) :
		...
		$total_liked_count = $liked_count = count( bp_activity_get_meta( $item_id, 'liked_count' ) );
		$output             = '';
...
function bp_like_button( $id = '', $type = '' ) {
	...
	if ( $type == 'activity' ) :
		...
		if ( is_user_logged_in() && $activity_type !== 'activity_liked' ) :
			...
			if ( !bp_like_is_liked( $bp_like_id, 'activity' ) ) : ?>
				<a href="#" class="<?php echo $bp_like_css; ?> bp-primary-action" id="like-activity-<?php echo $bp_like_id; ?>" title="<?php echo bp_like_get_text( 'like_this_item' ); ?>"><i class="fa fa-thumbs-o-up"></i> <?php echo bp_like_get_text( 'like' ); if ( $liked_count ) echo ' (' . $liked_count . ')'; ?></a>
			<?php else : ?>
				<a href="#" class="<?php echo $bp_like_css; ?> bp-primary-action" id="unlike-activity-<?php echo $bp_like_id; ?>" title="<?php echo bp_like_get_text( 'unlike_this_item' ); ?>"><i class="fa fa-thumbs-o-down"></i> <?php echo bp_like_get_text( 'unlike' ); if ( $liked_count ) echo ' (' . $liked_count . ')'; ?></a>
			<?php endif;
...
add_filter( 'bp_activity_entry_meta', 'bp_like_button', 1 );
add_action( 'bp_before_blog_single_post', 'bp_like_button', 1 );
add_filter( 'bp_activity_comment_options', 'bp_like_button', 1 );

/************** eyal *********************/

function bp_like_members( $id = '', $type = '' ) {
	...
	if ( $type == 'activity' ) :
		...
		if ( is_user_logged_in() && $activity_type !== 'activity_liked' ) :
			...
			if ( $users_who_like ): ?>
				<div class="bp-likes-members-wrapper">
					<?php echo '<i class="fa fa-thumbs-o-up"></i>'; bp_like_get_likes( $bp_like_id, 'activity', false); ?>
				</div>
			<?php
			endif;
		endif;
...
add_filter( 'bp_activity_entry_meta_after', 'bp_like_members',99 );
add_action( 'bp_before_blog_single_post', 'bp_like_members', 99 );
add_filter( 'bp_activity_comment_options', 'bp_like_members', 99 );
