...
/**
 * Created by PhpStorm.
 * Author: ritz <ritesh.patel@rtcamo.com>
 * Date: 16/9/14
 * Time: 6:57 PM
 */

class RTMediaProLicenseAPI {

	/**
	 * Self Upgrade Values
	 */
	// Base URL to the remote upgrade API Manager server. If not set then the Author URI is used.
	public $upgrade_url = 'https://rtcamp.com/';

	/**
	 * @var string
	 */
	public $version = RTMEDIA_PRO_VERSION;

	/**
	 * @var string
	 * This version is saved after an upgrade to compare this db version to $version
	 */
	public $api_manager_example_version_name = 'rtmedia_pro_am_version';
	...
	public $ame_update_check = 'rtmedia_pro_am_plugin_update_check';
	...
	public function __construct() {
		...
		register_activation_hook( RTMEDIA_PRO_PATH . 'index.php', array( $this, 'activation' ) );
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_software_product_id = 'rtMedia PRO';

			/**
			 * Set all data defaults here
			 */
			$this->ame_data_key 				= 'rtmedia_pro_am_data_key';
			$this->ame_api_key 					= 'rtmedia_pro_am_key';
			$this->ame_activation_email 		= 'rtmedia_pro_am_activation_email';
			$this->ame_product_id_key 			= 'rtmedia_pro_am_product_id';
			$this->ame_instance_key 			= 'rtmedia_pro_am_instance';
			$this->ame_deactivate_checkbox_key 	= 'rtmedia_pro_am_deactivate_checkbox';
			$this->ame_activated_key 			= 'rtmedia_pro_am_activated';

			/**
			 * Set all admin menu data
			 */
			$this->ame_deactivate_checkbox 			= 'rtmedia_pro_am_deactivate_checkbox';
			$this->ame_activation_tab_key 			= 'rtmedia_pro_am_dashboard';
			$this->ame_deactivation_tab_key 		= 'rtmedia_pro_am_deactivation';
			$this->ame_settings_menu_title 			= 'RTMedia PRO Subscription';
			$this->ame_settings_title 				= 'RTMedia PRO Subscription Settings';
			$this->ame_menu_tab_activation_title 	= __( 'License Activation', 'rtmedia' );
			$this->ame_menu_tab_deactivation_title 	= __( 'License Deactivation', 'rtmedia' );

			/**
			 * Set all software update data here
			 */
			$this->ame_options 				= get_option( $this->ame_data_key );
			$this->ame_plugin_name 			= RTMEDIA_PRO_BASE_NAME; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
			$this->ame_product_id 			= get_option( $this->ame_product_id_key ); // Software Title
			$this->ame_renew_license_url 	= 'https://rtcamp.com/my-account'; // URL to renew a license. Trailing slash in the upgrade_url is required.
		 ...
		 */
		register_deactivation_hook( RTMEDIA_PRO_PATH . 'index.php', array( $this, 'uninstall' ) );
	 ...
	 * API Key Class.
	 *
	 * @return RTMediaPro_API_Key
	 */
	public function key() {
		return RTMediaPro_API_Key::instance();
	}

	/**
	 * Update Check Class.
	 *
	 * @return RTMediaPro_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return RTMediaPro_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function activation() {
		...
		$api_manager_example_password_management = new RTMediaPro_API_Password_Management();
	...
	public static function am_example_inactive_notice() { ?>
		...
		<?php if ( isset( $_GET['page'] ) && 'rtmedia-license' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'The rtMedia PRO License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.', 'rtmedia' ), '<a href="' . esc_url( admin_url( 'admin.php?page=rtmedia-license' ) ) . '">', '</a>' ); ?></p>
		</div>
	<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'rtmedia' ), $this->ame_software_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
			<?php
			}
