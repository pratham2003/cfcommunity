...
new WPUpdatesThemeUpdater_949( 'http://wp-updates.com/api/2/theme', basename( get_template_directory() ) );
...
function basil_import_theme_options($json_file = false) {
	...
	if ($json_file){
	    ...
	    if ($_FILES["basil_import_file"]["error"] > 0) {
...
function basil_register_required_plugins() {
    ...
    $plugins = array(
    	...
    	array(
            'name'                  => 'Cooked',
            'slug'                  => 'cooked',
            'source'                => 'http://boxystudio.com/_plugin_cdn/cooked.zip',
            'required'              => true
        ),
		...
		array(
            'name'                  => 'Envira Gallery',
            'slug'                  => 'envira-gallery',
            'source'                => 'http://boxystudio.com/_plugin_cdn/envira-gallery.zip',
            'required'              => false
        ),
...
function crb_wp_enqueue_scripts() {
	...
	$custom_google_font = ot_get_option('to_general_custom_font','Lato');
  	crb_enqueue_style( TP . '-gf-custom', 'http://fonts.googleapis.com/css?family='.$custom_google_font.':100,200,300,400,500,600,700,800&subset=latin,cyrillic-ext,cyrillic,greek-ext,vietnamese,latin-ext');
	crb_enqueue_style(TP . '-font-awesome', TEMPLATE_DIR_URL . '/css/font-awesome.min.css');
	...
	if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
		crb_enqueue_style('jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/1.11.1/themes/smoothness/jquery-ui.css');
...
function basil_render_custom_css() {
	...
	$pattern_file = dirname(__FILE__) . '/lib/custom-basil-css-pattern.php';
