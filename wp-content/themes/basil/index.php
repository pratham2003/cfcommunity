				...
				if (!$disabled):
					...
					if(class_exists('cooked_plugin')) {
						...
						if (empty($recipeSlider)):
							...
							$recipe_query = new WP_Query( array('orderby' => 'date', 'order' => 'desc', 'post_type' => 'cp_recipe', 'posts_per_page' => 3) );
							...
							<div class="basilImageSlider">
								<?php foreach($recipeSlider as $slider):
									...
									if (!$private_recipe && $recipe_id): ?>
										<div class="basilSlide" style="background:url('<?php echo $recipe_image[0]; ?>') center center;"></div>
									<?php endif; ?>
								<?php endforeach; ?>
							</div>
							<div class="basilShell">
																...
																<div class="timing">
																	<ul>
																		<?php if (in_array('timing', $recipe_info) && $total_time) : ?>
																			<li><?php _e('Prep Time','basil'); ?>: <strong><?php echo cp_format_time($prep_time); ?></strong></li>
																			<li><?php _e('Total Time','basil'); ?>: <strong><?php echo cp_format_time($total_time); ?></strong></li>
																		<?php endif; ?>
