...
function buddyboss_scripts_styles()
{
	...
	/*
	 * Loads JavaScript file to add support for CSS3 media queries elements in older IE versions.
	 */
	global $is_IE;
	if ( $is_IE ) {
		wp_enqueue_script( 'respondjs', get_template_directory_uri() . '/js/respond.min.js', false, '1.1.0', false );
	}
	 ...
	 */
	if ( !is_admin() ) {
		wp_enqueue_script( 'touch-swipe', get_template_directory_uri() . '/js/jquery.touchSwipe.min.js', array('jquery'), '1.0', true );
		...
		wp_enqueue_script( 'buddyboss-mobile-main', get_template_directory_uri() . '/js/mobile-main.js', array('jquery'), '3.1.3', true );
	}
	...
	if ( $buddyboss->buddypress_active && !is_admin() )
	{
		...
		wp_enqueue_script( 'buddyboss-buddypress-js', get_template_directory_uri().'/js/buddypress.js', array( 'jquery' ), '3.1' );
	}
		...
		wp_localize_script( 'buddyboss-buddypress-js', 'BP_DTheme', $params );

		// Child themes can add variables by adding them to $buddyboss_js_params global
		$js_params = array_merge( $params, (array)$buddyboss_js_params );

	/*
	 * Adds custom BuddyBoss JavaScript functionality.
	 */
	if ( !is_admin() ) {
		wp_enqueue_script( 'buddyboss-main', get_template_directory_uri().'/js/buddyboss.js', array( 'jquery' ), '3.1' );
	}
	...
	if ( !is_admin() )
		wp_enqueue_style( 'buddyboss-wp-frontend', get_template_directory_uri().'/css/wordpress.css', false, '3.1.3', 'all' );
	...
	*/
	if ( $buddyboss->buddypress_active && !is_admin() )
	{
		...
		wp_enqueue_style( 'buddyboss-bp-frontend', get_template_directory_uri().'/css/buddypress.css', false, '3.1.2', 'all' );
	}
	...
	*/
	if ( $buddyboss->bbpress_active && !is_admin() )
	{
		...
		wp_enqueue_style( 'buddyboss-bbpress-frontend', get_template_directory_uri().'/css/bbpress.css', false, '3.1.2', 'all' );
	}

	// Load our CSS support for 3rd party plugins here.
	if ( !is_admin() ) {
		wp_enqueue_style( 'buddyboss-wp-plugins', get_template_directory_uri().'/css/plugins.css', false, '3.1', 'all' );
	}

	// Load our adminbar aka Toolbar styles.
	if ( !is_admin() ) {
		// Mobile
		wp_enqueue_style( 'buddyboss-wp-adminbar-mobile', get_template_directory_uri().'/css/adminbar-mobile.css', false, '3.1.2', 'all' );
		// Fixed or Floating (defaults to Fixed)
		wp_enqueue_style( 'buddyboss-wp-adminbar-desktop-'.esc_attr( get_theme_mod( 'boss_adminbar_layout', 'fixed' ) ), get_template_directory_uri().'/css/adminbar-desktop-'.esc_attr( get_theme_mod( 'boss_adminbar_layout', 'fixed' ) ).'.css', false, '3.1.1', 'all' );
	}
...
function buddyboss_scripts_jquery_migrate()
{
	if ( !is_admin() ) {
		...
		wp_register_script( 'jquery', get_template_directory_uri() . '/js/jquery.1.10.2.min.js', false, '1.10.2', false);
...
add_action( 'init', 'buddyboss_scripts_jquery_migrate', 0 );
...
/**
 * HTML5.js conditional
 *
 * @package  BuddyPress
 * @since    BuddyPress 3.0
 */
function buddyboss_print_html5_js()
{
...
add_action( 'wp_head', 'buddyboss_print_html5_js', -1 );

/**
 * respond.js conditional
 *
 * @package  BuddyPress
 * @since    BuddyPress 3.0
 */
function buddyboss_print_respond_js()
{
...
add_action( 'wp_footer', 'buddyboss_print_respond_js', -1 );
...
function load_fonts()
{
	if ( !is_admin() ) {
        ...
        // Google
        wp_register_style('googleFonts', 'http://fonts.googleapis.com/css?family=Open+Sans:300italic,400italic,600italic,700italic,400,700,600,300');
...
// add_filter( 'bbp_user_login_redirect_to', 'buddyboss_redirect_previous_page', 10, 3 );

function buddyboss_redirect_previous_page( $redirect_to, $request, $user )
{
	if(buddyboss_is_bp_active()){
		$bp_pages = bp_get_option('bp-pages');
		$activate_page_id = $bp_pages['activate'];
...
/**
 * Disable admin bar CSS except in Dashboard.
 * BuddyBoss loads its own admin bar stylesheet.
 *
 * @since BuddyBoss 3.0
 */
...
if ( !function_exists('buddyboss_addgravatar') ) {
	function buddyboss_addgravatar( $avatar_defaults ) {
		$myavatar = get_bloginfo('template_directory') . '/images/avatar-member.jpg';
