...
class BuddyBoss_Pics extends BP_Component
{
	...
	*/
	...
	public function setup_actions()
	{
		...
		add_action( 'after_setup_theme', array( $this, 'assets' ) );
	...
	function assets()
	{
		if ( !is_admin() ) { // no loading in the dashboard
			...
			// Styles
			wp_enqueue_style( 'buddyboss-pics-main', get_template_directory_uri() . '/buddyboss-inc/buddyboss-pics/css/buddyboss-pics.css', false, '3.1.2', 'all' );

			// Scripts
			wp_enqueue_script( 'buddyboss-pics-klass', get_template_directory_uri() . '/buddyboss-inc/buddyboss-pics/js/klass.min.js', array( 'jquery' ), '0.8.7', false );
			wp_enqueue_script( 'buddyboss-pics-popup', get_template_directory_uri() . '/buddyboss-inc/buddyboss-pics/js/code.photoswipe.jquery-3.0.5.min.js', array( 'jquery' ), '0.8.7', false );
			wp_enqueue_script( 'plupload', false, array( 'jquery' ) );
			wp_enqueue_script( 'plupload-html5', false, array( 'jquery' ) );
			wp_enqueue_script( 'plupload-flash', false, array( 'jquery' ) );
		}
	}
	...
	if ( !empty( $pics ) )
	{
		...
		foreach( $pics as $pic )
		{
			...
			if( $pic['privacy'] == 'loggedin' && !$bp_loggedin_user_id )
						continue;
			if( $pic['privacy'] == 'friends' && !friends_check_friendship( $bp_loggedin_user_id, $bp_displayed_user_id ) )
						continue;
...
function buddyboss_pics_input_filter( &$activity )
{
	...
	if ( strstr( $activity->content, 'class="buddyboss-pics-picture-link"' ) !== false
...
function buddyboss_pics_read_content_filter($filter)
{
	...
	if ( $image !== false && is_array( $image ) && count( $image ) > 2 )
	{
		...
		{
			$filter .= '<a class="buddyboss-pics-picture-wrap" href="'.$full[0].'">';
			$filter .= '<img data-permalink="'. bp_get_activity_thread_permalink() .'" class="buddyboss-pics-picture" src="'.$src.'"'.$width_markup.' /></a>';
		}
		else {
			$filter .= '<img data-permalink="'. bp_get_activity_thread_permalink() .'" class="buddyboss-pics-picture" src="'.$src.'"'.$width_markup.' /></a>';
		}
