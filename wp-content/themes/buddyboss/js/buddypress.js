...
jq(document).ready( function() {
		...
		BuddyBossSwiperClass = {
			...
			start: function() {
				...
				if ( $buddyboss_pic_grid.length === 1 )
				{
					...
					$buddyboss_pics = jq('.buddyboss-pics-picture-wrap');
				}
	...
	{
		var uploader_runtimes = 'html5,flash'; // 'html5,html4,flash'
		uploader = new plupload.Uploader({
			runtimes: uploader_runtimes,
			browse_button: 'whats-new-pic',
			container: 'whats-new-icons',
			max_file_size: '10mb',
			...
			flash_swf_url: BP_DTheme.tpl_url + '/buddyboss-inc/buddyboss-pics/assets/plupload.flash.swf',
			filters: [
				{title : "Upload a Picture", extensions : "jpg,jpeg,gif,png,bmp"},
			],
			init: {
	...
	jq("#aw-whats-new-submit").on( 'click', function() {
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				var $new = jq("li.new-update").find('.buddyboss-pics-picture-wrap');
...
//This fixes images disappearing when you click read more
		var a_inner = jq('#' + type + '-' + a_id + ' .' + inner_class + ':first p' );
